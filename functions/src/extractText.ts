  ...
  while ((match = btEtRegex.exec(content)) !== null) {
    ...
    while ((textMatch = textRegex.exec(block)) !== null) {
      const raw = textMatch[0].slice(1, -1)
        ...
        .replace(/\\t/g, "\t").replace(/\\\(/g, "(")
        .replace(/\\\)/g, ")").replace(/\\\\/g, "\\");
      if (raw.trim().length > 1) results.push(raw);
    }
  }
  if (results.length === 0) {
    const fallback = /\(([^\x00-\x1F\x7F-\xFF]{3,})\)/g;
    let fm;
    while ((fm = fallback.exec(content)) !== null) results.push(fm[1]);
  }
...
export const extractText = onCall(
  ...
  async (request) => {
    ...
    if (!type || !base64) throw new HttpsError("invalid-argument", "type et base64 requis.");

    // ── PDF → extraction locale SANS OpenAI ─────────────────────
    if (type === "pdf") {
      try {
        ...
        if (!text || text.length < 10) {
          throw new HttpsError("internal",
            "PDF vide ou non lisible. Essaie de prendre une photo du document.");
        }
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: "Extrais tout le texte visible dans cette image. Retourne uniquement le texte extrait, sans commentaire ni formatage supplémentaire." },
            ],
          }],
        });
        const text = response.choices[0]?.message?.content?.trim();
        if (!text || text.length < 5) throw new HttpsError("internal", "Aucun texte détecté.");
        return { text: text.slice(0, 8000) };
      } catch (e: any) {
        if (e instanceof HttpsError) throw e;
