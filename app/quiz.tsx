...
export default function QuizScreen() {
      ...
      const loadCache = async () => {
        try {
          ...
          if (raw) {
            ...
            if (data?.length > 0) { setCachedQuizzes(data); setHasMore(data.length >= PAGE_SIZE); }
          }
          const q = query(collection(db, "quizzes"), where("userId", "==", user.uid), orderBy("createdAt", "desc"), limit(PAGE_SIZE));
          ...
          const fromFirestore: CachedQuiz[] = snap.docs.map((doc) => ({
            ...
            score: doc.data().score || 0, total: doc.data().total || 0,
        ...
        } catch (e) { console.log("Quiz cache load error:", e); }
      };
      loadCache();
    }, [refreshTrigger])
  );
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const q = query(collection(db, "quizzes"), where("userId", "==", user.uid), orderBy("createdAt", "desc"), startAfter(lastDoc), limit(PAGE_SIZE));
      const snap = await getDocs(q);
    ...
    } catch (e) { console.log("Load more quizzes error:", e); }
  ...
  const handleSaveResult = async (finalScore: number) => {
    ...
    try {
      const newEntry: CachedQuiz = {
        id: Date.now().toString(), userId: user.uid, topic,
        score: finalScore, total: questions.length, questions,
        createdAt: new Date().toISOString(),
      };
      await addDoc(collection(db, "quizzes"), { ...newEntry, createdAt: Timestamp.now() });
      ...
      await addPoints("QUIZ_COMPLETED", { quizScore: finalScore, quizTotal: questions.length });
    } catch (e) { console.log("Quiz save error:", e); }
  };

  const handleReplay = (item: CachedQuiz) => {
    setTopic(item.topic); setQuestions(item.questions);
  ...
  const resultMessage = () => {
    if (score >= questions.length * 0.8)
      return currentLanguage === "ar" ? "ممتاز! أنت تتقن الموضوع." : currentLanguage === "en" ? "Excellent! You master the subject." : "Excellent ! Tu maîtrises le sujet.";
    if (score >= questions.length * 0.5)
      return currentLanguage === "ar" ? "جيد! واصل المراجعة." : currentLanguage === "en" ? "Good! Keep revising." : "Bien ! Continue à réviser.";
    return currentLanguage === "ar" ? "واصل جهودك، ستتقدم!" : currentLanguage === "en" ? "Keep up the effort, you'll improve!" : "Continue tes efforts, tu progresseras !";
  };
  ...
  const handleGenerate = async () => {
    if (topic.trim().length < 2) { Alert.alert(t("error"), "Saisis un sujet pour le quiz."); return; }
    ...
    startTracking(); setGenerating(true);
    try {
      ...
      endTracking(true);
    } catch (e: any) { endTracking(false); Alert.alert(t("error"), e.message || "Génération échouée."); }
  ...
  const handleNext = () => {
    if (currentIndex < questions.length - 1) {
      setCurrentIndex((i) => i + 1); setSelectedAnswer(null); setShowAnswer(false);
    } else { setPhase("result"); handleSaveResult(score); }
  };
  ...
  // Couleurs des options selon état dark/light
  const getOptionColors = (i: number) => {
    ...
    return { bg: C.card, border: C.borderMedium, textColor: C.text };
  };
      ...
      >
            ...
            onPress={() => {
              if (phase === "playing") {
                Alert.alert(
                  ...
                  currentLanguage === "ar" ? "هل تريد إلغاء الاختبار الجاري؟" : currentLanguage === "en" ? "Do you want to cancel the current quiz?" : "Veux-tu annuler le quiz en cours ?",
                  [
                    { text: currentLanguage === "ar" ? "متابعة" : currentLanguage === "en" ? "Continue" : "Continuer", style: "cancel" },
                    { text: currentLanguage === "ar" ? "إلغاء" : currentLanguage === "en" ? "Cancel" : "Annuler", style: "destructive",
                      onPress: () => { setPhase("setup"); setQuestions([]); setCurrentIndex(0); setScore(0); setSelectedAnswer(null); setShowAnswer(false); }
                    },
                  ]
                );
              } else { router.back(); }
          ...
          <View>
            ...
            <View style={{ flexDirection: isRTL ? "row-reverse" : "row", gap: 10, marginBottom: 28 }}>
              {["3", "5", "10"].map((n) => (
                ...
                >
                  <Text style={{ fontWeight: "600", fontSize: 15, color: count === n ? "#FFF" : C.text }}>
                    {n}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            {cachedQuizzes.length > 0 && (
              <View style={{ marginTop: 28 }}>
                ...
                {cachedQuizzes.slice(0, displayCount).map((item) => {
                  const isGood = item.score >= item.total * 0.8;
                  const isMed = item.score >= item.total * 0.5;
                  const scoreBg = isDark
                    ? (isGood ? "#022C22" : isMed ? "#2D1B00" : "#2D0A0A")
                    : (isGood ? "#F0FDF4" : isMed ? "#FFFBEB" : "#FEF2F2");
                  const scoreColor = isDark
                    ? (isGood ? "#34D399" : isMed ? "#FCD34D" : "#FCA5A5")
                    : (isGood ? "#065F46" : isMed ? "#92400E" : "#991B1B");
                  ...
                  return (
                    ...
                    >
                      <View style={{ flexDirection: isRTL ? "row-reverse" : "row", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <View style={{
                          backgroundColor: scoreBg, borderRadius: 8,
          ...
          <View>
            ...
            <View style={{
              ...
              <Text style={{
                ...
                lineHeight: 24, textAlign: isRTL ? "right" : "left",
            ...
            {q.options.map((option, i) => {
              const { bg, border, textColor } = getOptionColors(i);
              return (
                <TouchableOpacity
                  key={i}
        ...
        {phase === "result" && (
          <View style={{ alignItems: "center" }}>
            ...
            <Text style={{ fontSize: 15, color: C.textSecondary, marginBottom: 12, textAlign: "center" }}>
              {resultMessage()}
            </Text>
            ...
            <View style={{
              ...
              <Text style={{ fontSize: 12, color: isDark ? "#34D399" : "#065F46" }}>
                {currentLanguage === "ar" ? "تم حفظ النتيجة تلقائياً" : currentLanguage === "en" ? "Result saved automatically" : "Résultat sauvegardé automatiquement"}
              </Text>
            </View>
            ...
            <View style={{ width: "100%", gap: 12 }}>
              {/* Réessayer */}
              <TouchableOpacity
                onPress={() => { setCurrentIndex(0); setScore(0); setSelectedAnswer(null); setShowAnswer(false); setPhase("playing"); }}
                style={{
              ...
              <TouchableOpacity
                onPress={handleReset}
                style={{ backgroundColor: C.primary, borderRadius: 14, padding: 16, alignItems: "center", width: "100%" }}
              >
