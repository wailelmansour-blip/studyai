...
export default function PlanScreen() {
  ...
  const getShareContent = () => {
    ...
    const sessionsText = generatedPlan.sessions
      .map((s) => `📅 ${s.day} — ${s.subject} (${s.duration}h)\n${s.tasks?.map((task) => `  • ${task}`).join("\n") || ""}`)
      .join("\n\n");
    ...
    if (cached) {
      ...
      Object.entries(schedule).forEach(([day, items]: [string, any]) => {
        ...
        items.forEach((item: any) => {
          sessions.push({ day, subject: item.subject || "", duration: parseInt(item.duration) || 60, tasks: item.task ? [item.task] : [], completed: false });
        });
      });
      setGeneratedPlan({
        userId: auth.currentUser?.uid || "anonymous",
        subjects: limitedSubjects, examDate: examDate.toISOString(),
        totalDays: daysUntilExam(), sessions,
        title: cached.plan || `Plan — ${limitedSubjects.join(", ")}`,
        createdAt: new Date().toISOString(), tips: cached.tips || [],
      });
      return;
    }

    startTracking();
    setGenerating(true); setGeneratedPlan(null); setSaved(false);
    try {
      const generatePlanFn = httpsCallable(functions, "generatePlan");
      const input: GeneratePlanInput = { subjects: limitedSubjects, examDate: examDate.toISOString(), hoursPerDay: hours };
      ...
      const plan: StudyPlan = {
        ...
        createdAt: new Date().toISOString(), tips: data.tips || [],
      };
  ...
  const handleReset = () => {
    setGeneratedPlan(null); setSaved(false); setSubjects([""]);
    setExamDate(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)); setHoursPerDay("2");
  };

  const handleLoadFromHistory = (item: CachedPlan) => {
    setGeneratedPlan({
      userId: item.userId, title: item.title, subjects: item.subjects,
      examDate: item.examDate, totalDays: item.totalDays,
      sessions: item.sessions, tips: item.tips, createdAt: item.createdAt,
    });
    setSaved(true);
  };
          ...
          <View>
            ...
            <View style={{ marginBottom: 20 }}>
              ...
              {subjects.length < 8 && (
                <TouchableOpacity onPress={addSubject} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", marginTop: 4, alignSelf: isRTL ? "flex-end" : "flex-start" }}>
                  ...
                  <Text style={{ color: "#6366F1", marginLeft: isRTL ? 0 : 6, marginRight: isRTL ? 6 : 0, fontSize: 14, fontWeight: "500" }}>
                    {t("add_subject")}
                  </Text>
                </TouchableOpacity>
              )}
            ...
            <View style={{ marginBottom: 20 }}>
              ...
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: "#E5E7EB", borderRadius: 10, padding: 14,
                  flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between",
                }}
              >
                <Text style={{ fontSize: 15, color: "#111827" }}>{formatDate(examDate)}</Text>
            ...
            <View style={{ marginBottom: 28 }}>
              ...
              <View style={{ flexDirection: isRTL ? "row-reverse" : "row", gap: 10 }}>
                {["1", "2", "3", "4", "6"].map((h) => (
                  ...
                  >
                    <Text style={{ fontWeight: "600", fontSize: 15, color: hoursPerDay === h ? "#FFFFFF" : "#374151" }}>{h}h</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            ...
            >
              {generating ? (
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <Text style={{ color: "#FFFFFF", fontWeight: "700", fontSize: 16, marginLeft: 10 }}>{t("generating_plan")}</Text>
                </View>
              ) : (
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <Ionicons name="sparkles" size={20} color="#FFFFFF" />
                  <Text style={{ color: "#FFFFFF", fontWeight: "700", fontSize: 16, marginLeft: 8 }}>{t("generate_plan")}</Text>
                </View>
              )}
            </TouchableOpacity>
            ...
            {cachedPlans.length > 0 && (
              <View style={{ marginTop: 28 }}>
                ...
                {cachedPlans.slice(0, displayCount).map((item) => (
                  ...
                  >
                    <Text style={{ fontSize: 13, fontWeight: "600", color: "#374151", textAlign: isRTL ? "right" : "left" }} numberOfLines={1}>
                      🎯 {item.title}
                    </Text>
        ...
        {generatedPlan && (
          <View>
            <View style={{ backgroundColor: "#EEF2FF", borderRadius: 14, padding: 16, marginBottom: 20 }}>
              <Text style={{ fontSize: 17, fontWeight: "700", color: "#3730A3", marginBottom: 6, textAlign: isRTL ? "right" : "left" }}>
                🎯 {generatedPlan.title}
              </Text>
              <Text style={{ fontSize: 13, color: "#4338CA", textAlign: isRTL ? "right" : "left" }}>
            ...
            {generatedPlan.sessions.map((session: StudySession, index: number) => (
              <View key={index} style={{
                ...
                borderLeftColor: "#6366F1", borderRightColor: "#6366F1", elevation: 2,
              }}>
                <View style={{ flexDirection: isRTL ? "row-reverse" : "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <Text style={{ fontSize: 14, fontWeight: "700", color: "#111827", flex: 1, textAlign: isRTL ? "right" : "left" }}>
                    {session.day}
                  </Text>
                  <View style={{ backgroundColor: "#EEF2FF", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 }}>
                    <Text style={{ fontSize: 12, color: "#6366F1", fontWeight: "600" }}>{session.duration} h</Text>
                  </View>
                </View>
                <Text style={{ fontSize: 13, color: "#6366F1", fontWeight: "600", marginBottom: 6, textAlign: isRTL ? "right" : "left" }}>
                  📖 {session.subject}
                </Text>
                {session.tasks?.map((task: string, ti: number) => (
                  <View key={ti} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", marginTop: 4 }}>
                    <Text style={{ color: "#9CA3AF", marginRight: isRTL ? 0 : 6, marginLeft: isRTL ? 6 : 0, marginTop: 1 }}>•</Text>
                    <Text style={{ fontSize: 13, color: "#374151", flex: 1, textAlign: isRTL ? "right" : "left" }}>{task}</Text>
                  </View>
                ))}
              </View>
            ))}
            ...
            {(generatedPlan.tips ?? []).length > 0 && (
              <View style={{
                ...
                <Text style={{ fontSize: 14, fontWeight: "700", color: "#065F46", marginBottom: 8, textAlign: isRTL ? "right" : "left" }}>
                  💡 {t("ai_tips")}
                </Text>
                 {(generatedPlan.tips ?? []).map((tip: string, i: number) => (
                  <View key={i} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", marginTop: i > 0 ? 6 : 0 }}>
                    ...
                    <Text style={{ fontSize: 13, color: "#065F46", flex: 1, textAlign: isRTL ? "right" : "left" }}>{tip}</Text>
                  </View>
                ))}
              </View>
            )}
            ...
            <View style={{ flexDirection: isRTL ? "row-reverse" : "row", gap: 12, marginTop: 8, marginBottom: 12 }}>
              ...
              >
                <Text style={{ fontWeight: "600", color: "#374151", fontSize: 15 }}>🔄 {t("new_plan")}</Text>
              </TouchableOpacity>
              <TouchableOpacity
